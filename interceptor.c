...
//----- Helper Function 1 ----------------------------------------
// This helper must be declared before interceptor.
/**
 * Returns whether the given pid is monitored, regardless of whether the pid
 * list is a blacklist or whitelist.
 * Precondition: pid != 0
 */
static int check_pid_monitored_blwl(pid_t pid, int syscall) {
	if (table[syscall].monitored == 0) {
		return 0;
	} else if (table[syscall].monitored == 1) {
		return check_pid_monitored(syscall, pid);
	} else {
		return !check_pid_monitored(syscall, pid);
	}
}
...
asmlinkage long interceptor(struct pt_regs reg) {
	int syscall = reg.ax;
	int pid = current->pid;
	if (check_pid_monitored_blwl(pid, syscall)) {
		log_message(pid, (long unsigned int) syscall, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
	}
	return table[syscall].f(reg);
	// if check_pid_monitored_blwl
		// log registers etc
...
//----- Helper Functions Group 2 ---------------------------------

/**
 * Sets the pid to monitored status, regardless of whether the pid list
 * is a blacklist or whitelist.
 * If pid = 0 then set all pids to be monitored.
 * If pid is already monitored then return the appropriate error code
 * from the helper functions.
 */
static int set_pid_monitored(pid_t pid, int syscall) {
	spin_lock(&my_table_lock);
	if (pid == 0) {								// Set all monitored
		table[syscall].monitored = 2;
		destroy_list(syscall); 					// create empty blacklist
	} else if (table[syscall].monitored < 2) {	// add to whitelist
		if (add_pid_sysc(pid, syscall) != 0) return -ENOMEM;
		table[syscall].monitored = 1;
	} else {									// remove from blacklist
		if (del_pid_sysc(pid, syscall) != 0) return -EINVAL;
	}
...
/**
 * Sets the pid to unmonitored status, regardless of whether the pid list
 * is a blacklist or whitelist.
 * If pid = 0 then set all pids to be unmonitored.
 * If pid is already unmonitored then return the appropriate error code
 * from the helper functions.
 */
static int set_pid_unmonitored(pid_t pid, int syscall) {
	spin_lock(&my_table_lock);
	if (pid == 0) {								// Set all unmonitored
		table[syscall].monitored = 0;
		destroy_list(syscall); 					// create empty whitelist
	} else if (table[syscall].monitored == 1) {	// remove from whitelist
		if (del_pid_sysc(pid, syscall) != 0) return -EINVAL;
...
/**
 * Sets the syscall so that it is intercepted, replacing its caller function
 * with the generic interceptor function and saving the original.
 * If pid is already unmonitored then return the appropriate error code
 * from the helper functions.
 */
static void set_sysc_intercepted(int syscall) {
...
/**
 * Sets the syscall so that it is not intercepted and restores its original
 * caller function.
 * If syscall is already not intercepted do nothing.
 */
static void set_sysc_unintercepted(int syscall) {
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	// Cannot intercept or release MY_CUSTOM_SYSCALL or __NR_exit_group
	if (syscall == MY_CUSTOM_SYSCALL || syscall == __NR_exit_group) {
		return -EINVAL;
	}
	...
	// Check first two commands permissions
	if (cmd == REQUEST_SYSCALL_INTERCEPT || cmd == REQUEST_SYSCALL_RELEASE) {
		if (current_uid() != 0) {
			return -EPERM;
		}
	...
	// Check second two commands permissions
	if (cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING) {
		if(current_uid() != 0) {
	...
	// Cannot de-intercept an un-intercepted call
	if (cmd == REQUEST_SYSCALL_RELEASE && table[syscall].intercepted == 0) {
		return -EINVAL;
	}
	// Cannot stop monitoring and un-monitored or un-intercepted call
	if (cmd == REQUEST_STOP_MONITORING) {
		if (pid != 0 && !check_pid_monitored_blwl(pid, syscall)) {
			return -EINVAL;
	...
	// Cannot intercept an already intercepted call
	if (cmd == REQUEST_SYSCALL_INTERCEPT && table[syscall].intercepted == 1) {
		return -EBUSY;
	}
	// Cannot monitor an already monitored call
	if (cmd == REQUEST_START_MONITORING && check_pid_monitored_blwl(pid, syscall)) {
		return -EBUSY;
	}

	// If it reaches here the parameters are valid
	if (cmd == REQUEST_SYSCALL_INTERCEPT) {
		set_sysc_intercepted(syscall);
	} else if (cmd == REQUEST_SYSCALL_RELEASE) {
		set_sysc_unintercepted(syscall);
	} else if (cmd == REQUEST_START_MONITORING) {
		return set_pid_monitored(pid, syscall);
	} else if (cmd == REQUEST_STOP_MONITORING) {
		return set_pid_unmonitored(pid, syscall);
	}
...
{       
	// De-intercept syscalls
	int i;
	for (i = 0; i < NR_syscalls; i++) {
        my_syscall(REQUEST_SYSCALL_RELEASE, i, 0);
	}
	
	// Set special syscalls back to normal
	spin_lock(&sys_call_table_lock);
	set_addr_rw((unsigned long) sys_call_table);
